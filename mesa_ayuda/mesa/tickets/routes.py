...
def tickets_crear():
    ...
    if request.method == 'POST':
        ...
        numero_ticket = (request.form.get('numero_ticket') or '').strip()
...
def tickets_editar(ticket_id):
    ...
    if request.method == 'POST':
        ...
        db.execute(
            """
            UPDATE tickets SET
                            fecha_inicio=?, fecha_final=?, hora_inicio=?, hora_final=?, numero_ticket=?,
              sede=?, ubicacion=?,
              soporte_hardware=?, soporte_Software=?, soporte_redes=?,
              equipo_equipo=?, equipo_marca=?, equipo_modelo=?, equipo_cod_inventario=?, equipo_coin=?, equipo_disco=?, equipo_ram=?, equipo_procesador=?,
              servicio_tipo=?, servicio_otro=?, falla_asociada=?,
...
def tickets_descargar_csv():
    ...
    headers = [
        ...
        'equipo_cod_inventario', 'equipo_coin',
        'equipo_ram', 'equipo_disco', 'equipo_procesador',
        'servicio_tipo', 'servicio_otro', 'falla_asociada',
        'descripcion_solicitud', 'descripcion_trabajo',
        'eval_calidad_servicio', 'eval_calidad_informacion',
        'eval_oportunidad_respuesta', 'eval_actitud_tecnico',
        'firma_tecnico_mantenimiento_nombre', 'firma_usuario_gestiona_nombre',
        'firma_logistica_nombre', 'firma_supervisor_nombre',
    ...
    for r in rows:
        writer.writerow([
            r['id'], safe_cell(r['created_at']), safe_cell(r['creador']),
            safe_cell(r['fecha_inicio']), safe_cell(r['hora_inicio']),
            safe_cell(r['fecha_final']), safe_cell(r['hora_final']),
            safe_cell(r['sede']), safe_cell(r['ubicacion']),
            'Sí' if r['soporte_hardware'] else '',
            'Sí' if r['soporte_Software'] else '',
            'Sí' if r['soporte_redes'] else '',
            safe_cell(r['equipo_equipo']), safe_cell(r['equipo_marca']),
            safe_cell(r['equipo_modelo']),
            safe_cell(r['equipo_cod_inventario']), safe_cell(r['equipo_coin']),
            safe_cell(r['equipo_ram']), safe_cell(r['equipo_disco']),
            safe_cell(r['equipo_procesador']),
            safe_cell(r['servicio_tipo']), safe_cell(r['servicio_otro']),
            safe_cell(r['falla_asociada']),
            safe_cell(flatten(r['descripcion_solicitud'])),
            safe_cell(flatten(r['descripcion_trabajo'])),
            r['eval_calidad_servicio'] or '', r['eval_calidad_informacion'] or '',
            r['eval_oportunidad_respuesta'] or '', r['eval_actitud_tecnico'] or '',
            safe_cell(r['firma_tecnico_mantenimiento_nombre']),
            safe_cell(r['firma_usuario_gestiona_nombre']),
            safe_cell(r['firma_logistica_nombre']),
            safe_cell(r['firma_supervisor_nombre']),
...
def inventario_buscar():
    ...
    Consulta la BD MySQL de inventario y devuelve JSON.
    """
    import pymysql
    ...
    try:
        ...
        return jsonify(ok=False, error=f"No se pudo conectar a inventario: {e}"), 503
    ...
    try:
        with con:
            ...
            if coin:
                cur.execute("""
                    SELECT COIN AS coin, `COD INVENTARIO` AS cod_inventario,
                        TIPO AS equipo, MARCA AS marca, MODELO AS modelo,
                        RAM AS ram, `TAMAÑO SSD` AS disco,
                        PROCESADOR AS procesador, SEDE AS sede,
                        `UBICACIÓN` AS ubicacion
                    FROM inventario
                    WHERE UPPER(COIN) = UPPER(%s) LIMIT 1
                """, (coin,))
            else:
                cur.execute("""
                    SELECT COIN AS coin, `COD INVENTARIO` AS cod_inventario,
                        ...
                        `UBICACIÓN` AS ubicacion
                    FROM inventario
                    WHERE UPPER(`COD INVENTARIO`) = UPPER(%s) LIMIT 1
                """, (cod,))
            row = cur.fetchone()
    except Exception as e:
        return jsonify(ok=False, error=f"Error consultando inventario: {e}"), 500
