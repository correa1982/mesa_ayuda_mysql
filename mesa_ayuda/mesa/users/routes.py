...
def crear_usuario():
    ...
    if request.method == 'POST':
        ...
        # Firma (viene como data URL base64 desde el hidden input)
        firma_img = (request.form.get('firma_img') or '').strip()

        # Guardar
        pwd_hash = generate_password_hash(password)
        created_at = datetime.utcnow().isoformat()  # evita NOT NULL en created_at
        db.execute(
            "INSERT INTO users (username, password_hash, display_name, role, created_at, firma_img) VALUES (?, ?, ?, ?, ?, ?)",
            (username, pwd_hash, display_name, role, created_at, firma_img or None)
        )
        db.commit()
...
def editar_usuario(user_id):
    ...
    if request.method == 'POST':
        ...
        # Firma (opcional, viene como data URL base64)
        firma_img = (request.form.get('firma_img') or '').strip()

        # Actualizar
        if set_password and firma_img:
            db.execute(
                "UPDATE users SET display_name=?, role=?, password_hash=?, firma_img=? WHERE id=?",
                (display_name, role, set_password, firma_img, user_id)
            )
        elif set_password:
            db.execute(
            ...
            db.execute(
                "UPDATE users SET display_name=?, role=?, firma_img=? WHERE id=?",
                (display_name, role, firma_img, user_id)
            )
        else:
            db.execute(
