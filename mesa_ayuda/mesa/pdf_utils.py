...
def render_ticket_pdf(t, config=None):
    ...
    numero_ticket_raw = t.get("numero_ticket")
    numero_ticket_pdf = str(numero_ticket_raw).strip() if numero_ticket_raw is not None else ""
    if not numero_ticket_pdf:
        numero_ticket_pdf = "-"

    top -= 0.65 * cm
    _ = draw_label_value_tight(c, "Numero de ticket: ", numero_ticket_pdf, left, top, size=11, padding=3)
    ...
    # Ancho fijo por columna: se usa el máximo entre ambas etiquetas para
    # que los valores de "fecha/hora inicio" y "fecha/hora final" queden alineados
    _FB, _SZ = "Helvetica-Bold", 10
    _date_lw = max(stringWidth("Fecha inicio: ", _FB, _SZ),
                   stringWidth("Fecha final: ",  _FB, _SZ))
    _time_lw = max(stringWidth("Hora inicio: ", _FB, _SZ),
                   stringWidth("Hora final: ",  _FB, _SZ))
    _loc_lw  = max(stringWidth("Sede: ",      _FB, _SZ),
                   stringWidth("Ubicación: ", _FB, _SZ))

    # Fila 1: inicio
    c.setFont(_FB, _SZ)
    c.drawString(left, top, "Fecha inicio: ")
    c.setFont("Helvetica", _SZ)
    _v = val("fecha_inicio")
    c.drawString(left + _date_lw + 3, top, _v)
    x1_end = left + _date_lw + 3 + stringWidth(_v, "Helvetica", _SZ)
    x2 = max(col2_min, x1_end + min_gap_cols)
    c.setFont(_FB, _SZ)
    c.drawString(x2, top, "Hora inicio: ")
    c.setFont("Helvetica", _SZ)
    _v = val("hora_inicio")
    c.drawString(x2 + _time_lw + 3, top, _v)
    x2_end = x2 + _time_lw + 3 + stringWidth(_v, "Helvetica", _SZ)
    x3 = max(col3_min, x2_end + min_gap_cols)
    c.setFont(_FB, _SZ)
    c.drawString(x3, top, "Sede: ")
    c.setFont("Helvetica", _SZ)
    c.drawString(x3 + _loc_lw + 3, top, val("sede"))
    top -= line_gap

    # Fila 2: final
    c.setFont(_FB, _SZ)
    c.drawString(left, top, "Fecha final: ")
    c.setFont("Helvetica", _SZ)
    _v = val("fecha_final")
    c.drawString(left + _date_lw + 3, top, _v)
    x1_end = left + _date_lw + 3 + stringWidth(_v, "Helvetica", _SZ)
    x2 = max(col2_min, x1_end + min_gap_cols)
    c.setFont(_FB, _SZ)
    c.drawString(x2, top, "Hora final: ")
    c.setFont("Helvetica", _SZ)
    _v = val("hora_final")
    c.drawString(x2 + _time_lw + 3, top, _v)
    x2_end = x2 + _time_lw + 3 + stringWidth(_v, "Helvetica", _SZ)
    x3 = max(col3_min, x2_end + min_gap_cols)
    c.setFont(_FB, _SZ)
    c.drawString(x3, top, "Ubicación: ")
    c.setFont("Helvetica", _SZ)
    c.drawString(x3 + _loc_lw + 3, top, val("ubicacion"))
    top -= line_gap
    ...
    draw_label_value_tight(c, "Disco Duro:",         val("equipo_disco"),          col_x[2], top, size=10, padding=pad)
    ...
    # Firma Del Usuario (canvas) ✅ claves nuevas + fallbacks
    usr_img = get_first(
        t,
    ...
    slot(x2, y_sup, "Firma Del Usuario",                    usr_nom,      usr_img)
